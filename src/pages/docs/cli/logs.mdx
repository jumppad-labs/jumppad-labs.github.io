...
<Intro>
The logs command allows the tailing of logs from Jumppad resources.
</Intro>
...
```shell {{ title: "Example tailing logs for all running resources." }}
jumppad logs
...
```shell {{ title: "Example tailing logs for only the container resource named users-1." }}
jumppad logs container.users-1
