...
function ExecButton({ code }) {
  let [execCount, setExecCount] = useState(0)
  let executed = execCount > 0

  useEffect(() => {
    if (execCount > 0) {
      let timeout = setTimeout(() => setExecCount(0), 1000)
      return () => {
  ...
  return (
    <button
      type="button"
      className={clsx(
        'group/button relative rounded-full py-1 pl-2 pr-3 text-2xs font-medium',
        executed
    ...
    >
      <span
        aria-hidden={executed}
        className={clsx(
          'pointer-events-none flex items-center gap-0.5 text-zinc-400 transition duration-300',
          executed && '-translate-y-1.5 opacity-0'
        )}
      >
        <BoltIcon className="h-5 w-5 fill-zinc-500/20 stroke-zinc-500 transition-colors group-hover/button:stroke-zinc-400" />
        Exec
      </span>
      <span
        aria-hidden={!executed}
        className={clsx(
          'pointer-events-none absolute top-1 left-2.5 flex items-center justify-center text-emerald-400 transition duration-300',
          !executed && 'translate-y-1.5 opacity-0'
        )}
      >
        Executed
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden={!copied}
        className={clsx(
          'pointer-events-none absolute top-1 left-4 flex items-center justify-center text-emerald-400 transition duration-300',
          !copied && 'translate-y-1.5 opacity-0'
        )}
      >
        Copied
...
function CodeGroupHeader({ title, children, selectedIndex }) {
  ...
  return (
    <div className="flex flex-row min-h-[calc(theme(spacing.12)+1px)] items-start gap-x-8 border-b border-zinc-700 bg-zinc-800 px-4 dark:border-zinc-800 dark:bg-transparent">
      ...
      <div className="flex w-34 py-2.5 self-end justify-self-end gap-1.5">
        <ExecButton />
        <CopyButton />
      </div>
    </div>
